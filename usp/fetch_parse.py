...
class IndexRobotsTxtSitemapParser(AbstractSitemapParser):
    ...
    def sitemap(self, cutoff_date: Optional[datetime] = None) -> AbstractSitemap:
        ...
        for sitemap_url in sitemap_urls.keys():
            if cutoff_date:
                try:
                    year, month = extract_year_month_from_sitemap(sitemap_url)
                    sitemap_date = datetime(year, month, 1).astimezone(timezone.utc)

                    if sitemap_date < cutoff_date:
                        log.warning(
                            "Sitemap URL {} is older than cutoff date, skipping".format(
                                sitemap_url
                            )
                        )
                        continue
                    ...
                    log.warning(
                        "Unable to determine sitemap date for URL {}: {}".format(
                            sitemap_url, e
                        )
                    )
                    pass
...
class IndexXMLSitemapParser(AbstractXMLSitemapParser):
    ...
    def sitemap(self, cutoff_date: Optional[datetime] = None) -> AbstractSitemap:
        ...
        for sub_sitemap_url in self._sub_sitemap_urls:
            if cutoff_date:
                try:
                    year, month = extract_year_month_from_sitemap(sub_sitemap_url)
                    sitemap_date = datetime(year, month, 1).astimezone(timezone.utc)

                    if sitemap_date < cutoff_date:
                        log.warning(
                            "Sub-sitemap URL {} is older than cutoff date, skipping".format(
                                sub_sitemap_url
                            )
                        )
                        continue
        ...
        def page(self) -> Optional[SitemapPage]:
            ...
            if change_frequency:
                ...
                if SitemapPageChangeFrequency.has_value(change_frequency):
                    ...
                    log.warning(
                        "Invalid change frequency: {0}, defaulting to 'always'.".format(
                            change_frequency
                        )
                    )
        ...
        for page_row in self._pages:
            ...
            if page:
                if (
                    ...
                    and page.last_modified.astimezone(timezone.utc) < cutoff_date
                ):
                    continue
